...
if __name__ == "__main__":
    ...
    os.system("cd ../PhaseNet && git checkout master && git pull && cd ../scripts")

    run_phasenet(root_path=root_path, region=region, config=config)

    if num_nodes == 1:
        os.system(
            f"mv {root_path}/{region}/phasenet/phasenet_picks_{node_rank:03d}_{num_nodes:03d}.csv {root_path}/{region}/phasenet/phasenet_picks.csv"
        )
