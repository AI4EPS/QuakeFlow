...
def parse_time(time):
    ...
    time_obj = datetime.strptime(event_time, "%Y-%m-%dT%H:%M:%S.%f") + relativedelta(
...
events_located = events_located[
    (events_located["rms"] < rms_threshold)
    & (events_located["gap"] < gap_threshold)
    & (events_located["depth_km"] < 65)
]
