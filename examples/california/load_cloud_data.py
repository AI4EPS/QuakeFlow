...
if load_stations:
    ...
    for provider in providers:
        ...
        stations["begin_time"] = stations.apply(
            lambda x: datetime.strptime(x["begin_time"], "%Y-%m-%dT%H:%M:%S+00:00"), axis=1
        )
        stations["end_time"] = stations.apply(
            lambda x: datetime.strptime(x["end_time"], "%Y-%m-%dT%H:%M:%S+00:00"), axis=1
        )
        ...
        if selected_region:
            index = stations["longitude"].between(config["minlongitude"], config["maxlongitude"]) & stations[
                "latitude"
            ].between(config["minlatitude"], config["maxlatitude"])
            stations = stations[index]
        if selected_time:
            stations = stations[
                (stations["begin_time"] < config["endtime"]) & (stations["end_time"] > config["starttime"])
            ]
        ...
        stations["station_id"] = stations.apply(
            lambda x: f"{x['network']}.{x['station']}.{x['location']}.{x['instrument']}", axis=1
        )
        stations["begin_time"] = stations.apply(
            lambda x: datetime.strftime(x["begin_time"], "%Y-%m-%dT%H:%M:%S.%f"), axis=1
        )
        stations["end_time"] = stations.apply(
            lambda x: datetime.strftime(x["end_time"], "%Y-%m-%dT%H:%M:%S.%f"), axis=1
        )
        ...
        for station_id in stations["station_id"].unique():
            ...
            x_km, y_km = proj(
                station["longitude"].unique()[-1], station["latitude"].unique()[-1]
...
if load_catalog:
    ...
    for provider in providers:
        ...
        if provider == "NC":
            ...
            catalog_csv_info["year"] = catalog_csv_info.apply(
                lambda x: int(x["csv"].split("/")[-1].split(".")[0]), axis=1
            )
            catalog_csv_info["month_of_year"] = catalog_csv_info.apply(
                lambda x: int(x["csv"].split("/")[-1].split(".")[1]), axis=1
            )
        ...
        if provider == "SC":
            catalog_csv_info = pd.DataFrame(catalog_csv_list, columns=["csv"])
            catalog_csv_info["year"] = catalog_csv_info.apply(
                lambda x: int(x["csv"].split("/")[-1].split("_")[0]), axis=1
            )
            catalog_csv_info["day_of_year"] = catalog_csv_info.apply(
                lambda x: int(x["csv"].split("/")[-1].split("_")[1].split(".")[0]), axis=1
            )
        ...
        with ThreadPoolExecutor() as executor:
            events_list = list(
                tqdm(
                    executor.map(load_catalog_csv, catalog_csv_list),
                    total=len(catalog_csv_list),
                    desc=f"loading {provider}_catalog",
                )
            )
...
if load_picks:
    ...
    for provider in providers:
            ...
            for network in networks:
                ...
                for year in years:
                    for day_of_year in days_of_year:
                        csv_list_.extend(
                            fs.glob(f"{bucket_phasenet}/{folder_phasenet}/{network}/{year}/{year}.{day_of_year}/*.csv")
                        )
                with open(
                    f"{root_path}/{region}/data/picks/picks_csv_list/{provider}_{network}_csv_list.txt", "w"
                ) as fp:
                    fp.write("\n".join(csv_list_))
        ...
        if filter_picks_by_channel:
            ...
            csv_list = (
                csv_list.sort_values(by=["priority"]).groupby(["network", "station", "time"]).first().reset_index()
            )
        ...
        with ThreadPoolExecutor() as executor:
            picks_list = list(
                tqdm(executor.map(load_csv, csv_list["csv"]), total=len(csv_list), desc=f"loading {provider} picks")
            )
...
if merge_picks_by_cluster:
    ...
    picks_filt = pd.read_csv(
        f"{root_path}/{region}/data/picks/picks.csv", dtype={"phase_amplitude": str}
    )  # keep phase_amplitude as string to avoid float precision lost
    ...
    picks_filt["location_label"] = picks_filt["station_id"].map(
        lambda x: station_mapping[x] if x in station_mapping else None
    )
    picks_filt["timestamp"] = picks_filt.apply(
        lambda x: datetime.strptime(x["phase_time"], "%Y-%m-%dT%H:%M:%S.%f"), axis=1
    )
