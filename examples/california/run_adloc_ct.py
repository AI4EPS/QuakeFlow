...
if __name__ == "__main__":
    ...
    data_path = f"{root_path}/{region}/adloc_dd_2022"
    result_path = f"{root_path}/{region}/adloc_dd_2022"
    figure_path = f"{root_path}/{region}/adloc_dd_2022/figures"
    ...
    if ("xlim_km" not in config) or ("ylim_km" not in config) or ("zlim_km" not in config):
        ...
        zmax = config["maxdepth"] if "maxdepth" in config else 60
    ...
    for epoch in range(EPOCHS):
        ...
        valid_index = (
            np.abs(pred_time - pairs["phase_dtime"]) < np.std((pred_time - pairs["phase_dtime"])[valid_index]) * 5.0
        ...
        print(f"loss event time: mean {np.mean(time_shift):.3f}, std {np.std(time_shift):.3f}")
